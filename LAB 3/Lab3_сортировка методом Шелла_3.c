...
void function_sort_shell( int *array, int size )
{
  ...
  while( upper_sort > 0 )
  {
    for( int i = 0; i < size; i++ )
    {
      int cost = array[i]; // текущий элемент
      int j = i; // елемент по порядку в массиве
      while( (j >= upper_sort) && (array[j - upper_sort] > cost) ) // проверяем все елементы в отрезке приращивания, пока отстоющий элемент больше текущего 
      {
        array[j] = array[j - upper_sort]; // вкладывает в текущее место отстоющий элемент
        j = j - upper_sort;// следующий по отстованию
      }
      array[j] = cost; // сохроняем текущий элемент в не выявленное место
    }
